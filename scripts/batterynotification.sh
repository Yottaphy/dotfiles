...
while sleep 5; do
	...
	if [ "$status" = Discharging ] && [ "$capacity" -lt 5 ] && [ "$critbatnotsent" != yes ]; then
		critbatnotsent=yes
		mpv --no-config /usr/local/share/ding.mp3; notify-send 'Battery level critically low!' 'Suspending in 60 seconds.' &
		...
		lowbatnotsent=yes
		mpv --no-config /usr/local/share/ding.mp3; notify-send 'Battery low!' 'Plug in your charger now.' &
